...
async def search_single_platform(scraper, from_city, to_city, from_airports, to_airports, date, **kwargs):
    ...
    try:
        ...
        result = await scraper.search_flights(from_city, to_city, date, **kwargs)
...
def main():
    ...
    if not all([from_city, to_city, date]):
        print("用法: python skill.py <出发城市> <到达城市> <日期>")
        print("或: python skill.py --from <城市> --to <城市> --date <日期>")
        sys.exit(1)

    result = asyncio.run(compare_prices(from_city, to_city, date))
